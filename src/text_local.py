...
class ConcordanceIndex(object):
    ...
    def return_concordance(self, word, width=75, lines=25):
        ...
        if offsets:
            lines = min(lines, len(offsets))
            return_string = return_string + "Displaying %s of %s matches:\n" % (lines, len(offsets))
            for i in offsets:
                if lines <= 0:
                    break
                left = (' ' * half_width +
                ...
                left = left[-half_width:]
                right = right[:half_width]
                one_concordance =  "%s %s %s" % (left, self._tokens[i], right)
                return_string = return_string + one_concordance + "\n"
                lines -= 1
        else:
            return_string = "No matches\n"
